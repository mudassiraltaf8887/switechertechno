...
<body>
    <div class="container mt-5">
        ...
        <div class="d-flex justify-content-between align-items-center mb-4">
            <h2><i class="fas fa-edit"></i> Edit Lead #{{ $lead->id }}</h2>
            <a href="{{ route('manager.dashboard') }}" class="btn btn-secondary">
                <i class="fas fa-arrow-left"></i> Back to Dashboard
            </a>
        </div>

        <form action="{{ route('salesmanlead.store') }}" method="POST">
            @csrf
...
<div class="manager-section">
    ...
    <select class="form-select" name="manager_lead_priority" required>
        ...
        <option value="Low" {{ $lead->lead_priority == 'Low' ? 'selected' : '' }}>Low</option>
        <option value="Medium" {{ $lead->lead_priority == 'Medium' ? 'selected' : '' }}>Medium</option>
        <option value="High" {{ $lead->lead_priority == 'High' ? 'selected' : '' }}>High</option>
        <option value="Urgent" {{ $lead->lead_priority == 'Urgent' ? 'selected' : '' }}>Urgent</option>
    </select>
</div>

        <div class="col-md-6">
            <label class="form-label">Physical Demo Date <span class="required">*</span></label>
            <input type="datetime-local" class="form-control" name="physical_demo_date" 
                   value="{{ $lead->managerLead->physical_demo_date ?? '' }}" required>
        </div>
    </div>
    ...
    <div class="row">
        <div class="col-md-6">
            ...
            <select class="form-select" name="assigned_to_salesman" required>
                <option value="">Select Salesman</option>
                @foreach($salesmen as $salesman)
                    <option value="{{ $salesman->id }}" {{ ($lead->managerLead->assigned_to_salesman ?? '') == $salesman->id ? 'selected' : '' }}>
                        {{ $salesman->name }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="col-md-6">
            ...
            <textarea class="form-control" name="manager_remarks" rows="3" placeholder="Manager's notes...">{{ $lead->managerLead->manager_remarks ?? '' }}</textarea>
        </div>
    </div>
</div>
            <!-- Salesman Input Section -->
            <div class="salesman-section">
                <div class="salesman-title">
                    <i class="fas fa-user-check"></i> Salesman Input
                </div>

                <div class="row">
                    <div class="col-md-6">
                        <label class="form-label">DEMO <span class="required">*</span></label>
                        <select class="form-select" name="demo_type" required>
                            <option value="">Select Demo Type</option>
                            <option value="DEMO ON VISIT">DEMO ON VISIT</option>
                            <option value="ONLINE DEMO">ONLINE DEMO</option>
                            <option value="CLIENT NEED QUOTATION">CLIENT NEED QUOTATION</option>
                        </select>
                    </div>
                    ...
                    <div class="col-md-6">
                        ...
                        <select class="form-select" name="status" required>
                            ...
                            <option value="WAITING BY CLIENT">WAITING BY CLIENT</option>
                            <option value="WAITING BY SALE PERSON">WAITING BY SALE PERSON</option>
                        </select>
                    </div>
                </div>
                ...
                <div class="row">
                    ...
                    <div class="col-md-4">
                        <label class="form-label">AMOUNT QUOTATED</label>
                        <input type="number" class="form-control" name="amount_quotated" step="0.01" placeholder="Enter amount">
                    </div>

                    <div class="col-md-4">
                        <label class="form-label">REMARKS</label>
                        <textarea class="form-control" name="salesman_remarks" rows="3" placeholder="Salesman's notes..."></textarea>
                    </div>
                </div>
            </div>
